...
class SingleReadBucketSuite extends FunSuite {
  ...
  test("convert unmapped pair to fragment") {
    ...
    assert(fragment.getAlignments.size === 0)
    assert(fragment.getSequences.size === 2)
    assert(fragment.getSequences.get(0).getBases === "ACACACAC")
    assert(fragment.getSequences.get(0).getQualities === "********")
    assert(fragment.getSequences.get(1).getBases === "GTGTGTGT")
    assert(fragment.getSequences.get(1).getQualities === ";;;;++++")
  ...
  test("convert proper pair to fragment") {
    ...
    assert(fragment.getSequences.get(0).getBases === "ACACACAC")
    assert(fragment.getSequences.get(0).getQualities === "********")
    assert(fragment.getSequences.get(1).getBases === "ACACACAC")
    assert(fragment.getSequences.get(1).getQualities === "++++;;;;")
  ...
  test("convert read pair to fragment with first of pair chimeric read") {
    ...
    val fragment = srb.toFragment
    assert(fragment.getSequences.size === 2)
    assert(fragment.getSequences.get(0).getBases === "ACACACACACACAC")
    assert(fragment.getSequences.get(0).getQualities === "********654321")
    assert(fragment.getSequences.get(1).getBases === "ACACACAC")
    assert(fragment.getSequences.get(1).getQualities === "++++;;;;")
